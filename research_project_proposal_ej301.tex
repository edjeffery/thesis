...
\nomenclature{Data Quality Objective}{DQO}
...
\begin{table}[!tb]
  \centering
  \caption{GPIO pin layout and usage}
  \label{gpio}
  \begin{tabular}{ l l l }
  \toprule
...
\begin{table}[!tbp]
  \centering
  \caption{Comparison of particulate sensors considered}
...
\end{subfigure}
\caption{Solution to sensor attachment problem.}
\end{figure}
...
The power to the device was supplied by a 10000mAh lithium-ion battery. We measured the current draw of the whole device using an ammeter, dissecting a USB cable and attaching clips to the voltage common collector (VCC) wire. This test setup is shown in Figure \ref{fig: ammeter}, with the dissected cable shown in Figure \ref{fig: usb_cable}. The ammeter reported current values between 748mA and 754mA. Therefore, the device could run for approximately 13 hours on a fully-charged battery. Equation \ref{eqn: time} is only an estimate because batteries typically have non-linear discharge due to factors such as temperature and age of battery.
...
\begin{subfigure}[b]{0.4\textwidth}
  ...
  \label{fig: usb_cable}
\end{subfigure}
\caption{Power consumption testing}
\end{figure}

\subsection{Case}

The case is a modified version of a triangular bag that fits to the frame of a bicycle. It has three velcro straps that secure the bag to the bicycle's frame. The inside of the bag is lined with a flexible plastic, thus helping to make the device water-resistant. Holes were cut into the side of the bag to expose the air quality sensors to the air, to allow the GPS module to have a clear view of satellites in the sky and to display a system-status LED to the user.

\nomenclature{LED}{Light emitting diode}

\subsection{Cost}

One of the main aims of this project was to create a low-cost device. In total, the cost was \textsterling 152.63. A breakdown is shown in Table \ref{costs}.

\begin{table}[!tbp]
  \centering
  \caption{Cost breakdown of device}
  \label{costs}
...
\begin{enumerate}
\item Software that runs on the Raspberry Pi to collect and send the sensor data
\item Server-side software that receives the data and inserts it into a database
\item Web application that displays the data to the public
\end{enumerate}
...
The main requirement of the software on the Raspberry Pi was to read the data from the sensors periodically and then send this data to a specified server. The basis for this software came from analysing code from the ENLITEN project [REF], a previous study that the University of Bath was involved in, as well as from \cite{thorpe2017RPimesh}.

Although \cite{thorpe2017RPimesh} used the same particulate sensor, the study used a Grove Pi HAT, which was based around an Arduino and the software only allowed for measurements every 30 seconds. We worked with the manufacturer Grove to reduce this, but encountered numerous problems with re-flashing the Grove Pi with altered firmware. To overcome this, we instead attached the sensor to the GPIO pins and used the \texttt{pigpio} library, which provides control of the RPi's GPIO. Furthermore, we used the \texttt{spidev} library for communicating with the MQ131 and MQ135 sensors (more specifically, their ADC chips). \texttt{spidev} is a python module for interfacing with SPI devices via the spidev linux kernel driver. One problem we experienced was reading from all of the sensors simultaneously, because the continual nature of reading from the particulate sensor (measuring how long the signal was low for in a given time period) was `blocking' the main thread. As a resolution, we created two threads: one for reading the particulate sensor and a second for the rest of the sensing functionality. This avoided any blocking and the sensor measurements were saved to variables in the relevant thread objects. Every 10 seconds, these variables were read by the main program and saved to a CSV file. Once the CSV file had 6 lines of data, corresponding to 1 minute, we started a new file.

To avoid manual intervention by the user, the \texttt{/etc/rc.local} (`rc' stands for `run-control') was edited to include a start service command, shown below. We tested using \texttt{cron}, but found it difficult to get working on reboot.
...
For sending the data, we saved an Android phone's WiFi hotspot details in the Raspberry Pi's \texttt{wpa-supplicant.conf} file, which meant it would automatically connect when it detected the WiFi network. An example config file is shown in Appendix XXX. We then used the \texttt{requests} python module for sending the CSV file via HTTP post requests over the RPi's WiFi connection.

These two requirements satisfied the basic functionality of the RPi software. However, for robustness, the software stored un-sent data on the RPi until it could be sent again. For example, if the WiFi connection failed, then the files were stored in a separate directory and then all sent when the connection was restored. In addition, the software detected potentially erroneous measurements (i.e. a zero reading) and alerted the user by flashing a red LED. This was achieved through the use of the \texttt{RPi-GPIO} library. 
...
Initially, we developed an Android application that merged readings from the RPi with GPS data because the RPi does not have built-in GPS abilities. The application was created using Android Studio for Mac OSX. The RPi connected to the Android phone via Bluetooth, and used \texttt{ObexFTP} to send data files from the RPi to the phone. The Android application was watching a particular directory for new files and upon receipt would match up timestamps in the file with GPS-timestamp pairs stored on the phone, before HTTP posting the file to the server. However, this was a far from ideal situation and introduced tight coupling between the RPi and the phone. It was realised after the development of this application that USB GPS modules were available that could plug directly into the RPi.

\nomenclature{FTP}{File Transfer Protocol}

Although not used in the final version of the device, this development shows that it is possible for the device to connect via Bluetooth and to share the internet connection of another device via Bluetooth Tethering. It also demonstrates the ability to still collect GPS location data when the Raspberry Pi does not have GPS itself and no GPS module is available.

The GPSD protocol, which is built on top of JSON, was used to communicate with the GPS receiver. Responses from the GPS receiver are JSON objects. The \texttt{gpsd-py3} module is a Python module that interfaces with the GPSD protocol, parsing the JSON to output longitude and latitude coordinates, as well as other useful information such as speed and altitude. 

Moreover, the RPi was configured to use the GPS receiver as a Network Time Protocol (NTP) time server. This involves the use of the NTP daemon. Installation instructions are detailed in the Appendix. By enabling this, the RPi could sync its internal clock with an accurate source -- the GPS satellites. This overcomes the issue of the Raspberry Pi having no real time clock, which can lead to inaccurate timing when recording measurements.
...
\subsection{Server-side software and database}

In order to connect the air quality monitoring device and the database, we set up a client-server architecture. We used a RESTful API to receive files and then inputted the file's data into a database.
...
The University of Bath provides a database hosting service to members of the university. The database software is MySQL version 5.7.16 Community Server. A database was created for this project that contained two main tables: (i) a table that stored the air quality data, and (ii) a table that stored system monitoring information. We were able to just use one table for storing the measurements as there were not enough rows to cause performance issues. The system monitoring table allowed us to identify if and when an error occurred in the data flow.

% Describe tables here

\subsection{Web application}

The web application was developed using R Shiny version 1.1.0. An example page from the web application is shown in Figure \ref{web_app}.

\begin{figure}[!tb]
\centering
\includegraphics[width=1\textwidth]{web_app}
\caption{Web application using R Shiny}
\label{web_app}
\end{figure}

The web page's main requirement was to display the information in easily accessible and understandable manner so that any member of the public would be able to analyse the pollution in their area. It predominantly displays the pollution maps of Bath and line graphs for the time series of data. On the left-hand side of the page is a menu bar that allows the user to select their chosen time period, with the graphs dynamically updating as the selection changes. In addition, there is a dynamic table that displays all of the raw data, with a button for downloading the dataset as a CSV file.
...
Therefore, the mass of particles per cubic foot can be calculated by multiplying the mass of a particle $m$ by the number of particles per cubic foot $y$. It is then a simple conversion from cubic feet to cubic metres, by multiplying by a factor of $3531.5$.
\begin{equation}
...
The output of the MQ131 ozone sensor is a raw analog signal that represents voltage, with the voltage proportional to the quantity of the gas being sensed. Given that the RPi cannot read an analog signal (digital only), an ADC chip was included on the Mikro sensor board. The ADC used was an MCP3551, which is a single-channel 22-bit ADC chip. The communication between the RPi and this ADC is via a serial communication. More specifically, a Serial Peripheral Interface (SPI). This is a synchronous serial communication interface consisting of an SPI master and SPI slave relationship, detailed in Figure \ref{spi}.

\begin{figure}[!tb]
\centering
\includegraphics[width=0.8\textwidth]{spi_master_slave}
\caption{A single master and a single slave on a Serial Peripheral Interface (SPI) bus.}
\label{spi}
\end{figure}
...
\caption{Typical Serial Device Communication and Example Digital Output Codes for Specific Analog Input Voltages [REF]}
\label{mcp3551}
\end{figure}

Once the RPi has read the ADC value, this value can then be converted to a decimal value using a binary-to-decimal conversion. The output voltage of the sensor is a value between 0 and 5V. Since the range of decimal values is \SI{-2097152} to \SI{2097151}, the decimal value $d$ needs to be converted to a fraction and then multiplied by 5V to get the voltage output value $V_{IN}$.
\begin{equation}
V_{IN} = \frac{d + 2097152}{4194304} \cdot V_{REF}, \hspace{5mm} V_{REF} = 5V
\end{equation}

Using the calibration curves on the MQ131 sensor data sheet (Figure \ref{mq131_curve}), it is possible to calculate an approximate ozone reading in $\mu g/m^3$. The first part of the calculation is based on the circuit structure, shown in Figure \ref{circuit}.
% Insert circuit diagram here
\begin{figure}[!htb]
\centering
\includegraphics[width=0.6\textwidth]{circuit}
\caption{Circuit diagram for MQ-series sensors}
\label{circuit}
\end{figure}

First, we had to work out the resistance of the sensor in clean air (R\textsubscript{0}). Since no controlled ozone environment was available for calibration of the sensor, we assumed that the indoor air was clean and contained minimal ozone. To achieve this, we set the sensor's potentiometer to its minimum ($0\Omega$) and recorded the output voltage $V_L$. We then also set the potentiometer to its maximum ($10k\Omega$) and recorded the output voltage $V_H$. Figure \ref{potentiometer} shows the potentiometer of both the MQ131 and MQ135 sensors. Once the output voltage at these two points was recorded, it was possible to calculate R\textsubscript{0} by using the following equation:
\begin{equation}
...
\end{equation}
In fact, we have two equations since we have two different pairs of $R_L$ and $V_{OUT}$.
\begin{equation}
R_0 = (\frac{5}{4.934} - 1) \cdot 10100 = 135.103 , \quad R_0 = (\frac{5}{2.015} - 1) \cdot 100 = 148.139
\end{equation}
Theoretically, these calculated $R_0$ values should be the same, but here we take the average as they differ slightly.
\begin{equation}
...
\caption[Potentiometers]{Left: blue potentiometer of ozone click (MQ131). Right: blue potentiometer of air quality click (MQ135).}
\label{potentiometer}
\end{figure}
...
\end{equation}
This enables the calculation of the ratio R\textsubscript{S}/R\textsubscript{0}, which can be inputted into the calibration curve (Figure \ref{mq131_curve}) to output an approximate ozone reading in $\mu g/m^3$. We digitised the calibration curve to obtain $(x, y)$ pairs and then used power regression to obtain the final equation:
\begin{equation}
...
\begin{subfigure}[b]{0.45\textwidth}
  ...
  \caption{Calibration curve from MQ131 data sheet.}
  \label{mq131_curve}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.45\textwidth}
  \includegraphics[width=\textwidth]{mq135_data_sheet}
  \caption{Calibration curve from MQ135 data sheet.}
  \label{mq135_curve}
\end{subfigure}
...
Furthermore, we unfortunately did not have access to known concentrations of ozone gas and so could not measure the output voltage of the MQ131 sensor at specific concentrations of ozone. As a result, we could not plot points to create our own calibration curve. Instead, we had to use the data sheet's calibration curve, which may be inaccurate.

Similarly for the MQ135, we did not have access to known concentrations of NOx in the sensor's detection range. We did manage to test the sensor output using 500~ppm NOx gas in the university's Automotive Research Group laboratory. However, this concentration is far greater the normal range of NOx found in the atmosphere and so overwhelmed the sensor. Moreover, the aforementioned issue relating to the sensor's cross-sensitivities meant we could not calibrate the voltage to a specific concentration of a single gas. This meant the device could not pin-point locations of high NOx, but nonetheless was still useful to identifying poor general air quality.

\nomenclature{ppm}{Parts-per-million}

Moreover, because these sensors are low-cost, they have cross-sensitivities with other atmospheric gases that can potentially cause unwanted changes in sensor output. However, without the correct testing equipment and more expensive sensors that only have very minimal cross-sensitivities, it was not possible to correct for these effects in the given time frame. This is unfortunately a common issue when using low-cost sensors, hence their price.

It is also worth noting that the data sheets for these sensors indicate that their outputs can drift over time, which has also been found by previous studies [REF]. Our data collection period was 2 weeks, which is too short to notice a change as they typically occur over bi-annual or annual periods of time. However, this effect should be tested and accounted for if the device were to be used for longer periods of time.
...
As Chapter \ref{chap: related_work} concluded, there are a number of gaps in the current literature surrounding air quality monitoring devices. Using these gaps, we have narrowed the focus of this study into answering three main research questions. These are as follows:
\begin{enumerate}
\item Can air pollution be monitored to a satisfactory level using a bicycle-based monitoring device?
%
% Position on bike
% Waterproof
% Vibrations
...
% Low-cost sensors
%
\item Can this device improve spatial and temporal resolution of air quality measurements compared to fixed monitoring stations?
% Locus data
% Event-based data
\item How can these air quality monitoring devices be applied for public benefit?
% 
\end{enumerate}
...
\caption{Elevation raster of Bath}
\label{bath_elevation}
\end{figure}

\section{Data cleaning and data validation}

Whilst ideally our sensors would not output anomalous readings, it is almost inevitable given their low cost. Therefore, it is prudent to detail our approach to cleaning the data outputted by these sensors, especially for future reproducibility.
...
In order to more precisely identify which measurements are outliers in our data, we used a two-part approach. In each case, we applied the method and if one or more of these methods reported a reading to be an outlier, it was discarded from the sample.
\begin{enumerate}
\item Inter-quartile range (IQR)
\item Median and Median Absolute Deviation (MAD)
\end{enumerate}

These outlier detection techniques were then used to create a type of band pass filter in which data outside a calculated range were filtered out.

\subsection{IQR} \label{iqr}

The IQR was created by \cite{tukey1977iqr} and provides a measure of statistical dispersion. It is calculated as the difference between the 25th and 75th percentiles. Coupled with the 25th and 75th percentiles ($Q_1$ and $Q_3$, respectively), it can be used to define an outlier detection rule. Values outside the range shown in Equation \ref{eqn:iqr} are considered outliers.
...
Since our dataset is a collection of time series, the outlier detection should also take this into account. There are two ways we incorporated this into our methodology.

Firstly, the outlier detection techniques detailed above were also run using moving averages. The dataset was separated into distinct collection runs (e.g. 2018-08-12 AM) and the moving average outlier detection was run on each of these. The reasoning for using a moving average was to account for any trends in the pollution data. For example, if, on a main road, the pollution level was much higher than the average for that collection run, it would not make sense to use the outlier detection methods on the dataset as a whole as otherwise these higher values could be removed erroneously (i.e. false positive).

The second way we accounted for the time series nature was using a similar method to \cite{vanZoest2018outlierdetection}. The authors appear to be one of the first to create a method for identifying outliers in spatial-temporal air quality data. Their method consists of separating the data into spatial and temporal divisions. For example, urban traffic and urban background (spatial), and weekdays/weekends and traffic times (temporal). The reason for this was that air quality can depend highly on the local area and the time of day, so outlier detection must take these divisions into account. As a result, the spatio-temporal variability in pollution concentrations is maintained. Our data cannot be split easily into spatial categories as we have focused on a small city, but we did separate the data into 4 temporal categories. The first split was for a weekday or weekend. The rationale behind this was traffic is highly likely to be higher during the week when people are commuting to and from work. The second split was for the first half of the day (AM) and the second half of the day (PM). When combined, these gave four categories:
\begin{enumerate}
\item Weekday AM
\item Weekday PM
\item Weekend AM
...
The route was selected based upon the current fixed air quality monitoring stations in Bath, so that measurements obtained by this project could be compared to the official council measurements. The locations of these stations are displayed and labelled in Figure \ref{cycling_route}.
...
\begin{figure}[!tb]
  ...
  \caption{Map of fixed air quality monitoring sites in Bath with the chosen cycling route overlaid.}
  \label{cycling_route}
\end{figure}
...
Furthermore, BANES outlined an `Air Quality Management Area' in 2013. This is shown in Figure \ref{aqma}. Again, the cycling route almost completely falls within this area, thus strengthening our rationale behind the route's choice.
...
A bicycle was ridden with the air quality monitoring device attached around a pre-determined route, detailed in Section \ref{route_selection}. The route was cycled in both directions in order to test whether there were any differences in pollution depending on direction of travel. This led to minor differences in the route due to certain one-way streets in Bath. The bicycle was ridden at a relatively constant speed in order to distribute measurements regularly in spatial terms.

Data was collected across 3 weeks in July and August, from Monday 23rd July to Tuesday 14th August. The route was cycled both in the morning and afternoon. The morning run was to coincide with the morning rush hour and the evening run was to coincide with the evening rush hour. These times were deemed to have the highest pollution levels across the day. 3 weeks was also deemed long enough to allow for weather variations, which allowed us to analyse the effect of weather on the measurements. Over \num{10000} observations were collected across these 3 weeks of data collection, with an average of \num{450} observations collected each day.
...
With the pollution data collected along the route detailed in Section \ref{route_selection}, we created pollution maps that showed how pollution varied along the route on average, at different times of day and different times of the week. To achieve this, we used hexagonal shapes based upon \cite{Hoang2013hanoihexagons}. Each hexagon's colour is an average of all values that fall inside it's boundaries. Additionally, \cite{smallbone2012customerinsight} found that the public prefer a blue or green colour for representing good air quality and red for representing poor air quality. Consequently, we also follow this colour scheme.

\subsection{Positioning on bicycle} \label{position_testing}

In order to identify which position on a bicycle is optimal for an air quality monitoring device, we need to test the device attached to different positions. We identified three potential positions on a bicycle, shown in Figure \ref{bicycle_positions}. Position 1 is situated above the front wheel, thus avoiding any particles thrown up by the wheel. In addition, it is the most suitable position for the user to view the device and allows the user to notice when the system-status LED is flashing. Position 2 is situated directly below the rider and could potentially be impacted by particles thrown up by the front wheel. It is also not as visible to the user. On the other hand, this may provide good protection from the elements and help to make the device robust. Finally, Position 3 is the least visible, but is situated lowest on the bicycle frame and, thus, provides a different testing environment for the sensor.

\begin{figure}[!tb]
\centering
\includegraphics[width=0.6\textwidth]{images/bicycle_positions}
\caption{Potential positions for device on bicycle.}
\label{bicycle_positions}
\end{figure}

We will cycle with the device attached in each position in turn along the same stretch of road. The results will then be compared to ascertain whether the position of the device makes a difference. We will compare variability of measurements and the mean values.


\subsection{Air flow testing}

As Section \ref{air_flow} detailed, air flow can affect the sensor measurements. In particular, light-scattering particulate monitoring, which the Shinyei PPD42 sensor uses, relies on sufficient air flow to pass particles through a beam of light. As a result, our air flow testing focuses on the particulate sensor. We propose three different air flow systems for this device:
...
In order to answer this research question, we firstly need to demonstrate how the device can collect measurements across a greater area compared to a fixed monitoring station, whilst not sacrificing temporal resolution. By collecting measurements across a larger area, it is possible to generate a more detailed view of pollution in an area, thus improving spatial resolution. Secondly, we need to demonstrate how improved temporal resolution is possible without sacrificing spatial resolution. 

\subsection{Locus around fixed monitoring station}

A fixed monitoring station can only take measurements at exactly one location. Whilst they do typically use high-quality, expensive monitoring equipment that offers higher accuracy and reliability, they have poor spatial resolution. Our bicycle-based air quality monitoring device is portable and so can take measurements at varying locations with ease. To demonstrate this improvement in spatial resolution, we selected a suitable location and data collection method and then analysed the results.
...
To test the event-based capabilities of the device, we will cycle along the route before, during and after the sporting event, thus allowing comparison of air quality across these three distinct time periods. In each time period, we will cycle continuously back and forth along the route, which should take approximately 2 minutes in each direction.

\begin{figure}[!tb]
\centering
\includegraphics[width=0.6\textwidth]{images/bath_fc_route}
\caption{Event-based route outside of Bath City FC football stadium.}
\label{fig: bath_fc_route}
\end{figure}

\subsubsection{Demonstrating higher temporal resolution}

The data should provide a pollution level along the route for every 5 minute interval, which we can then use to show how the pollution changes over the whole time period. In order to demonstrate the high temporal resolution, 
...
Bath's roads have a total length of 52.8~km. Given the resource constraints of this project, it was deemed infeasible to collect pollution data across the whole city whilst only using one cyclist. Moreover, collecting any more than one time period of data would be even more difficult. Consequently, we simulated the pollution data across Bath so that we had values to apply in the routing algorithm. We sectioned the city into a raster of 100 by 100 cells, assigning a pollution value to each cell.

However, whilst this simulation of data is sufficient for this project, we wanted to understand how commuting cyclists could be used in a city to collect the pollution data that could then be fed into the routing algorithm. The foremost question here is how many cyclists would be needed to generate sufficient coverage of a city. To test this, we generated random origin and destination pairs, plotting the shortest route between the two points. From these random routes, we can calculate the percentage of a road network covered. This was calculated for number of cyclists $n=1$ up to $n=500$.

Using the most recent census data \citep{ons2011census}, the urban area of Bath had a population of $94782$ and 1.9\% of the population cycled to work. This means there are an estimated $94782 \times 1.9\% = 1801$ people that commute via bicycle in Bath. Clearly, not all of these commutes will be within the boundaries of our test area, so testing up to $n=500$ is not an unreasonable figure.

In order to minimise the number of trivial routes, we introduced a minimum commute distance of 500~m. This avoids the origin and destination being extremely close to each other.
...
We then combined these assumptions with three variables we believed would affect the health impact of travelling along a road. The distance of the road, from our assumption, clearly impacts how long an individual is exposed to the pollution, so we include the product of the length and the pollution level. This product was used as a cost function by \cite{Hasenfratz2015highresmapsTram}, which is shown in Equation \ref{eqn:hasenfratz}. The routes generated by this cost function decreased pollution exposure by 7.1\% on average.
...
Since data collection for this application was deemed infeasible within the project's constraints, the data was generated by sampling points from a distribution. The distribution of pollution measurements in the main route dataset was used to make the values as realistic as possible.
...
Once the required number of `health-optimal' and shortest route pairs had been generated, we compared these routes to analyse the percentage decrease in pollution exposure and the percentage increase in distance.
...
\begin{table}[!tbp]
  ...
  General AQ & 9877 & 1.17 & 0.21 & 1.60 & 0.74 & 195 & 1.97 \\ \bottomrule
    \multicolumn{7}{l}{\textsuperscript{$\ast$}\footnotesize{Lower bound (L.B.) capped at 0.}} \\
  \end{tabular}
