...
\title{\vspace{0cm}A low-cost bicycle-mounted air quality monitoring system using a Raspberry Pi, with a pollution-avoidance routing application}
...
\begin{abstract}
Air pollution is a major health problem, causing millions of deaths each year, as well as many other non-fatal diseases  \citep{WHO2016ambient}. In order for authorities to be able to mitigate air pollution effectively, they need access to high-resolution air quality data. Currently, air quality monitoring in the UK depends on fixed monitoring stations, which offer poor resolution, both spatially and temporally. In this paper, we design and construct an air quality monitoring device that attaches to a bicycle, thus providing a mobile platform for air quality data collection in the city of Bath. We show that it can monitor air quality to a satisfactory level, with the collected data being used to create pollution maps and identify trends in air pollution. We also show the device's improvements in spatial and temporal resolution compared to fixed monitoring stations, as well as its ability to rapidly be deployed to major events for air quality monitoring. Furthermore, we present a novel cost function and routing algorithm that finds `health-optimal' routes, decreasing pollution exposure by 5.08\% on average compared to the shortest routes.
\end{abstract}
...
Thank you also to my trusty bicycle, which only suffered 3 punctures throughout the whole project, despite the hundreds of potholes that litter Bath's roads.
...
\item whether air quality can be monitored to a satisfactory level using a bicycle-based monitoring device:
...
The sensitivity of this sensor is a clear drawback if accurate measurements of PM\textsubscript{2.5} are needed. However, we are more interested in creating a working air quality monitoring device and testing the effectiveness of a bicycle-based device, meaning a lower accuracy can be afforded. Moreover, PM\textsubscript{10} is still a governmentally monitored pollutant in the UK (see Table \ref{airqualityguidelines}) and provides a solid measure of air quality. Nonetheless, it is still worth noting this issue for future developments.
...
\newline\newline
\noindent\textit{\textbf{Research Question 1:} Can air pollution be monitored to a satisfactory level using a bicycle-based monitoring device?}
\newline
...
There does not appear to be any previous research on this topic, potentially because there are very few bicycle-based air quality monitoring devices. Therefore, there are no other findings to compare these results against.
...
Answering this question will help to ascertain whether such bicycle-based devices can replace their more expensive, fixed counterparts. Section \ref{meth:q2} details the methodology behind answering this research question and then Section \ref{results:q2} displays the results and analyses the findings.
