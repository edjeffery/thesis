...
% Front matter
%\title{\vspace{-1cm}A low-cost bicycle-mounted air quality monitoring system using a Raspberry Pi, with a pollution-avoidance routing application}
\title{\vspace{0cm}A low-cost portable pollution monitoring system}
...
\begin{document}
\newgeometry{left = 3.5cm, right = 3.5cm, top = 4cm, bottom = 2cm}
\pagenumbering{roman}
\maketitle

\begin{abstract}
According to the World Health Organisation, air pollution is a major health problem, causing millions of deaths each year, as well as many other non-fatal diseases. In order for authorities to be able to mitigate air pollution effectively, they need access to high-resolution air quality data. Currently, air quality monitoring in the UK depends on fixed monitoring stations, which offer poor resolution, both spatially and temporally. In this dissertation, we report on the design and construction of an air quality monitoring device that attaches to a bicycle, thus providing a mobile platform for air quality monitoring. This device has been trialled at various sites around the city of Bath in order to collect air pollution data for analysis. We show that the device can monitor air quality to a satisfactory level of accuracy, with the collected data being used to create pollution maps and identify trends in air pollution. We also show the device's improvements in spatial and temporal resolution compared to fixed monitoring stations, as well as its ability to rapidly be deployed to major events for air quality monitoring. Furthermore, we present a novel cost function and routing algorithm that calculates health-optimal routes, decreasing pollution exposure by 5.08\% on average, compared to the shortest routes in the city of Bath.
\end{abstract}

\renewcommand{\abstractname}{Acknowledgements}
\begin{abstract}
I would first like to thank my research project supervisor, Dr. Julian Padget, for his guidance and assistance throughout this project and during the course. 
...
Martin Fullick (Department of Physics) was a great help with the soldering and circuit-related issues. His knowledge was invaluable.

Thank you also to my trusty bicycle, which only suffered three punctures throughout the whole project, despite the hundreds of potholes that litter Bath's roads.
...
\begin{itemize}
\item We design and create a bicycle-mounted air quality monitoring device that measures particulates, ozone and general air quality (Chapter \ref{chap: system_design}).
...
\item We test whether air pollution can be monitored to a satisfactory level using a bicycle-mounted monitoring device (Chapter \ref{chap:q1}). The results show that this is achievable, with the device demonstrating sensitivity to trends in air pollution, as well as the creation of pollution maps from the data collected by the device. In addition, we also identify the optimal position of the monitoring device on a bicycle.
...
In addition, Chapter \ref{chap:conclusion} concludes the project and details the limitations of our work. It also provides recommendations for future work, as well as an additional note on a Mongolian project for which the device is being used.
...
This research project to design and build a low-cost air quality monitoring system covers a wide number of topics. Given the relative infancy of low-cost sensors and low-cost computing, making such a system has only been made possible in recent years and, as such, the literature needs thorough investigation to understand where the current implementations can be improved to make widespread air quality monitoring a more realistic possibility. 

Firstly, the health impacts of air pollution are considered, outlining why being able to monitor air pollution easily is a major concern. The various measures of air quality are then detailed in order to establish an understanding of what are the most important pollutants to measure. Next, previous monitoring devices are listed and the most relevant studies are evaluated to identify where novel research is needed and where improvements on past studies can be made. Potential candidates for the base computer of the proposed air quality monitoring device are then compared. Subsequently, portable power solutions for the device are considered. The specific drawbacks of using low-cost sensors are then detailed, as well as a discussion of techniques in the literature for improving them. Finally, methods for mapping air quality data and monitoring the data quality are analysed. 
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.7\textwidth]{images/Rationale_for_air_quality_monitoring}
\caption[Rationale for air quality monitoring]{Rationale for air quality monitoring \citep{WHO2002ambientmonitoring}}
\label{rationaleformonitoring}
\end{figure}

%This paper was instrumental in raising awareness of the need for air quality monitoring. 

Air pollution is formed of gases and microscopic particles, which are often invisible to the naked eye. Therefore, in order for any pollution reduction strategies to be effective, there needs to be a method of measuring and monitoring air pollution so that problem areas can be identified.
...
Looking at the air quality monitoring literature as a whole, a number of areas remain unaddressed. Firstly, no study measures all of pollutants detailed in Section \ref{airqualitymeasures} that form UK and EU legislation. Whilst this may be because it is difficult to fit that number of sensors into one device, if mobile air quality monitoring in the future is to be a viable option in the UK, this needs to be addressed. Secondly, the capabilities of using a sole Raspberry Pi device for air quality monitoring purposes needs further investigation as it is often used in conjunction with another computer (see Table \ref{listofprevdevices}). Thirdly, only three of the studies detailed in Table \ref{listofprevdevices} used bicycles, which are ubiquitous, non-polluting vehicles that would appear to be an ideal candidate vehicle for mobile air quality monitoring. Finally, in all of these three cases, the air quality monitoring device appeared bulky and cumbersome (see Figure \ref{aeroflex} for an example). Therefore, it would be interesting to see if all of the sensing capabilities could be incorporated into a small device.
...
A number of studies have shown that air pollution in cities can vary over extremely short distances ($<100$~m) \citep{hewitt1991spatialvar, wilson2006intraurbanvar}. Therefore, it is likely that air pollution measurements from a fixed monitoring station are only representative of a small radius around the site. This acts as a trade-off with the improved accuracy that fixed monitoring stations provide. Furthermore, with these stations typically located in areas of high pollution that the local authorities wish to monitor, the measurements may provide an inflated estimate if taken to be representative of a large area.

It has also been noted that the presence of unbroken rows of tall buildings on either side of a road can trap air pollution \citep{vardoulakis2002streetcanyon}. These types of roads have been named `street canyons'. In addition, the level of pollution behind these buildings can drop to background levels, indicating the dramatic variability in pollution that can occur in a city.
...
Conversely, a monitoring device attached to a moving vehicle -- i.e. a \textit{mobile} device -- can cover a larger area for a lower cost since a much smaller number of devices needs to be used \citep{Wallace2009mobilehamilton}. The area being monitored is also flexible as a moving device does not have the same restrictions as a fixed one. Moreover, setting a suitably short interval for taking air quality measurements enables the moving device to have a greater spatial resolution as many measurements can be taken in close proximity to one another. For example, \cite{Apte2017googlestreetview} took measurements every 30 metres and found using a mobile system led to a $10^4$ to $10^5$ times greater -- i.e. finer -- spatial resolution compared to fixed monitoring stations. In addition, \cite{Hasenfratz2015highresmapsTram} combined pollution data collected by trams with pollution modelling to generate maps with a high spatial resolution of $100~m \times 100~m$.

The apparent advantages of mobile air quality monitoring over static monitoring suggest it is more suitable for this project. In addition, a collective statement about the future of European air quality monitoring recommended mobile devices for `the collection of highly spatially and temporally resolved data' \citep{Kuhlbusch2014futureEUair}.

\subsection{Temporal resolution}

Much of the air quality literature relating to the resolution of measurements focuses on the spatial resolution. However, the temporal resolution is also important as a higher temporal resolution allows one to better understand how the pollution in an area changes over time. \cite{Elen2013aeroflex} mapped a short route in Antwerp, Belgium, with a temporal resolution of 1 hour. Using this high temporal resolution data, the authors found that pollution was highest during the morning rush hour, with concentrations decreasing steadily afterwards. In a different study, \cite{Hasenfratz2015highresmapsTram} managed to generate pollution maps of the city of Zurich with a semi-daily temporal resolution. This demonstrates that the temporal resolution is lower (i.e. less granular) when a larger area is covered. 

\cite{Peters2013cycleruns} use a novel heatmap visualisation to illustrate temporal changes in pollution. An example is displayed in Figure \ref{fig:temporal_viz}. The y-axis is the `run' number (i.e. temporal unit) and the x-axis is the name of the street. This is a useful visualisation as it is easy for the reader to notice pollution changes over time.

\begin{figure}[!t]
\centering
\includegraphics[width=0.6\textwidth]{images/temporal_viz}
\caption[Temporal pollution change visualisation.]{Novel visualisation of temporal pollution changes \citep{Peters2013cycleruns}.}
\label{fig:temporal_viz}
\end{figure}

Unfortunately, there is often a trade-off between spatial and temporal resolution. Improvements in spatial resolution lead to a reduction in temporal resolution because any given location will only have multiple measurements once the moving device visits that exact location again \citep{Wong2009envmonitoringtemporal}. One solution to this problem is to increase the number of vehicles deployed with the monitoring device attached, which increases the frequency of measurements at any given location. 
...
\begin{figure}[!tbp]
\centering
\includegraphics[width=0.6\textwidth]{images/LURexplanation}
...
\begin{figure}[!tb]
\centering
\includegraphics[width=0.7\textwidth]{images/googleapi_heatmap_layer}
...
An alternative is to use a custom coloured shape overlay on a map. For instance, \cite{Hoang2013hanoihexagons} used hexagons, shown in Figure \ref{hanoihexagons}. The hexagonal shapes were used instead of the more widely used rectangular shapes because of their visual advantages. Firstly, there is less map ambiguity, and secondly, hexagonal shapes are easier for human vision to interpret since, on the map, there are no distinct horizontal and vertical lines, to which the authors note human vision is especially sensitive. 

\begin{figure}[!tb]
\centering
\includegraphics[width=0.7\textwidth]{images/hanoihexagons}
...
\section{Comparison of single-board computers} \label{comparison}

Whilst this section does not intend to provide a comprehensive comparison of single-board computers (SBCs) for use in this project (this would be a significant project itself), it is important to consider the relative advantages and disadvantages of various SBCs.

\nomenclature{SBC}{Single-board computer}

The decreasing cost and size of computers over time has created a market for low-cost, SBCs that are compact, but have significant computing capabilities. There are various makes of SBCs available, but the most commonly used SBCs in the air quality monitoring literature are compared in this section. Whilst an Arduino is technically a microcontroller unit (MCU), its prevalence in the literature means it should also be included in this comparison.
...
One issue that Raspberry Pis have is that they do not have an onboard clock and so the system time is the time since the last boot. To alleviate this problem, a time source is needed to accompany the RPi. Furthermore, the RPi cannot read an analog signal, which a number of pollution sensors output. As a result, an analog-to-digital converter (ADC) would need to be used.
...
However, a major problem is that the data generated by these low-cost sensors is often of questionable accuracy and reliability. The causes of this are varied \citep{Clements2017lowcostworkshop}. Firstly, low-cost sensors are often sensitive to   environmental conditions such as humidity and temperature. Secondly, the sensor readings are known to change as they age, with sensor drift leading to the frequent need to recalibrate, thus reducing their cost advantage. Moreover, they are sensitive to the air flow, which is particularly poignant given the prevalence of mobile air quality monitoring deployments. Finally, the low-cost sensors can be exhibit a delayed response to a change in the pollution level and a sensor for one pollutant can exhibit cross-sensitivity to a variety of other pollutants. As a result of these issues with low-cost sensors, calibration techniques have been developed to help improve the data quality.

\subsection{Calibration of sensors} \label{calibration_techniques}
...
Another machine learning technique uses random forest-based machine learning algorithm. \cite{zimmerman2018machinelearning} were the first to apply this technique to low-cost air quality sensor calibrations. Again, it was found that a machine learning technique (random forests) outperformed linear regression models and was able to make the low-cost sensors accurately characterise air pollution concentrations synonymous with European city levels. Moreover, by including measurements of additional pollutants, the random forest model was able to account for pollutant cross-sensitivities, with which \cite{Spinelle2015fieldcalibrationa,Spinelle2017fieldcalibrationb} struggled with. This highlights the need of having multiple different sensors in the same device.

Whilst it appears machine learning methods are the best available methods for low-cost sensor calibration, the aforementioned studies used stationary air quality monitoring. Thus, it is unclear how these methods would be applied to a mobile air quality monitoring device.

\subsection{Air flow} \label{air_flow}

Since low-cost sensors are sensitive to the air flow, it is important that the sensors are provided with a consistent air flow so as to not affect their measurements. One study has noted the benefit of air flow in improving air quality measurements from low-cost sensors. \cite{thorpe2017RPimesh} found that the increased air flow from using an integrated fan improved the signal to noise ratio of a particulate sensor. The fan provided a more consistent air flow to the sensor, which may have been the reason for the improvement. However, the author noted that battery drain may have led to a variable fan speed, which in turn could have affected the measurements. This implies that variability in air flow may need to be controlled for. In addition, \cite{Hasenfratz2015highresmapsTram} used a fan installed in the back of their air quality monitoring `box' to draw air out and ensure a steady air flow.

Alternatively, a mobile device may receive sufficient air flow as the vehicle moves. Unfortunately, a literature search did not discover any research relating to this application. Therefore, this should be researched further.
...
\item whether air quality can be monitored to a satisfactory level using a bicycle-mounted monitoring device:
%\item whether a number of sensing capabilities can be incorporated into one, sufficiently small device;
\begin{enumerate}
	\item mapping air pollution accurately from low-cost sensor data;
	\item finding the optimal position on a bicycle for an air quality monitoring device;
	\item assessing the air flow needs of bicycle-mounted sensors;
	\item applying outlier detection methods to air quality data;
\end{enumerate}
\item whether such a device can improve spatial and temporal resolution in comparison to fixed monitoring stations; and
...
The core of the sensing device is a Raspberry Pi Zero W single-board computer, with a 1 GHz CPU running the Raspbian Stretch, Linux-based, operating system\footnote{Link to download: \href{https://www.raspberrypi.org/downloads/raspbian/}{https://www.raspberrypi.org/downloads/raspbian/}}. The sensing capabilities come from a Shinyei PPD42 particulate sensor, a MikroElektronika Ozone 2 Click, a MikroElektronika Air Quality Click and a DHT22 temperature and humidity sensor. The MikroElektronika sensors are connected to the RPi using a MikroElektronika Pi 3 Click Shield. A GlobalSat BU-353S4 USB GPS receiver supplies the precise geospatial information for geo-tagging the air quality measurements. The built-in WiFi of the RPi is used to connect to a WiFi hotspot in order to upload the measurements to a MySQL database (version 5.7.16 Community Server) via a PHP script (see Appendix \ref{code:php_api}). An Android mobile phone running Android 6.0.1 is used to provide the WiFi hotspot whilst cycling. The program running on the RPi is written in the Python programming language and can be found in Appendix \ref{code:main_program}. The power for the device is supplied via a 10000mAh external battery pack, with a 5V 2.4A output. The web application is written in R and uses R Shiny to generate the interface. The web application code is detailed in Appendix \ref{code:web_app}.

\nomenclature{GPS}{Global positioning system}

\subsection{Raspberry Pi}

The air quality monitoring device is built upon a Raspberry Pi Zero W. This RPi model is the smallest available, with a footprint of only 65~mm by 30~mm, and so helps to minimise the overall footprint of the device. As discussed in Section \ref{comparison}, the RPi is a computer, rather than a microcontroller, which enables additional functionality on top of running the sensor reading software for this project. For example, one can use SSH to remotely access the command line of the RPi, as well as use SCP to move files on and off the device, without having to physically connect it to another computer.

It is worth noting that the RPi Zero W did encounter overheating issues during the project, which rendered one RPi completely useless. The device's CPU was running extremely hot, which is potentially a side-effect of the device containing a large number of sensors. Unfortunately, RPis do not come with heat-sinks, but these could be included in future designs. We tested using a Raspberry Pi 2 instead, but settled on a replacement RPi Zero W, which did not encounter any further issues. The Raspberry Pi 3 was also tested as an alternative, but the Serial Peripheral Interface (SPI) setup is different and so the software did not run correctly (see Section \ref{rpi_software} for SPI details). 
...
The chosen Shinyei PPD42NS sensor has two output lines, named P1 and P2. P1 is used to measure particles between $1\mu m$ and $10\mu m$, whereas the P2 output is used to measure particles between $2.5\mu m$ and $10\mu m$. In testing, we rarely registered a signal from P2 that differed from P1, which would indicate that the sensor only detects particles with a diameter above $2.5\mu m$. This implies the output from the sensor is classified as PM\textsubscript{10}. This agrees with the findings of \cite{thorpe2017RPimesh}, who found that the sensor responded very poorly to smoke which has an average particle size of between 1 and 2$\mu m$.

The sensitivity of this sensor is a clear drawback if accurate measurements of PM\textsubscript{2.5} are needed. However, we are more interested in creating a working air quality monitoring device and testing the effectiveness of a bicycle-mounted device, meaning a lower accuracy can be afforded. Moreover, PM\textsubscript{10} is still a governmentally monitored pollutant in the UK (see Table \ref{airqualityguidelines}) and provides a solid measure of air quality. Nonetheless, it is still worth noting this issue for future developments.

\subsection{Ozone sensor}

For the gas sensors, we opt for using MikroElektronika's click-board technology, which provides a homogeneous platform into which a variety of sensors can be plugged directly into without the need for soldering. This dictates our choice of ozone sensor because MikroElektronika only has one option available -- the `Ozone 2 Click'. This click-board uses an MQ131 sensor on the circuit board, which has a tin dioxide sensing layer that reduces in conductivity in clean air. It can detect ozone concentrations between 10 and 1000 parts-per-billion (ppb).
...
A number of previous studies use the MQ135 sensor for air quality monitoring purposes. \cite{Rahman2017adaptivesensingRPi} used the MQ135 in an Internet-of-Things (IoT) based platform, whilst \cite{anderson2012noxdroid} used it on a bicycle-mounted air quality monitoring device. In addition, \cite{garnier2017mythorreality} tested the MQ135 and found it to be accurate for monitoring changes and trends in air quality.
...
One of the main aims of this project is to create a low-cost device. In total, the cost of a complete air quality monitoring device is \textsterling 150.43. A breakdown is shown in Table \ref{costs}.

\begin{table}[!tbp]
  \centering
  \caption{Cost breakdown of device. The overall cost of the device is relatively low at only \textsterling 150.43, which satisfies this project's objective of making a low-cost air quality monitoring device.}
  \label{costs}
  \begin{tabular}{ l S[table-format=3.2] }
  \toprule
  Item & {Cost (\textsterling)} \\ \midrule
  Raspberry Pi Zero W & 12.80  \\
  Micro SD Card (8GB) & 5.00 \\
...
The main requirement of the software on the Raspberry Pi is to read the data from the sensors periodically and then send this data to a specified server. The basis for this software comes from analysing code from the ENLITEN project \citep{mogles2017enliten}, a previous study that the University of Bath was involved in, as well as from \cite{thorpe2017RPimesh}.

Although \cite{thorpe2017RPimesh} evaluated the same particulate sensor, the study used a Grove Pi HAT to connect the sensor to the RPi, which is a device based on an Arduino and the software only allowed for measurements every 30 seconds. We worked with the manufacturer Grove to reduce this, but encountered numerous problems with re-flashing the Grove Pi with altered firmware. To overcome this, we attach the sensor to the GPIO pins instead and use the \texttt{pigpio} python module, which provides control of the RPi's GPIO. Furthermore, we use the \texttt{spidev} library for communicating with the MQ131 and MQ135 sensors (more specifically, their ADC chips). \texttt{spidev} is a python module for interfacing with SPI devices via the \texttt{spidev} linux kernel driver. A problem can arise when attempting to read from all of the sensors simultaneously, because the continual nature of reading from the particulate sensor (measuring how long the signal was low -- i.e. 0 -- for in a given time period) `blocks' the main thread. As a resolution, the software implements two threads: (i) reading the particulate sensor and operating the LED, and (ii) operating the rest of the sensing functionality. This avoids any blocking and the sensor measurements are saved to variables in the relevant thread objects, which can then simply be accessed from the object. Every 10 seconds, these variables are read by the main program and saved to a CSV file. Once the CSV file has 6 lines of data, corresponding to 1 minute of time, the software starts a new file.
...
A RESTful API written in PHP receives the HTTP posted CSV file, then parses the contents and inserts each line into a row of the database table that stores all measurements. If this completes with no errors, it then also inserts a new row into a system monitoring table with information about the CSV file and time of insertion. If there is an error, a flag is raised in the table that can then be reviewed to ascertain the cause. Appendix \ref{code:php_api} shows the API code.

\nomenclature{REST}{Representational State Transfer}

\subsubsection{Database}

The University of Bath provides a database hosting service to members of the university. The database software is MySQL version 5.7.16 Community Server. One of these databases is used in this project, containing two main tables: (i) a table that stores the air quality data, and (ii) a table that stores system monitoring information. We are able to use only one table for storing the measurements as the number of rows is low enough not to cause performance issues. The system monitoring table allows identification if and when an error occurs in the data flow.
...
\begin{figure}[!tb]
\centering
\includegraphics[width=0.8\textwidth]{images/spi_master_slave}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=1\textwidth]{images/mcp3551}
\caption[MCP3551 output codes.]{Summary of output coding data format. This shows example digital output codes for specific analog input voltages when using a typical SPI communication \citep{mq131datasheet}}
\label{mcp3551}
\end{figure}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.6\textwidth]{images/circuit}
\caption[Circuit diagram for MQ-series sensors.]{Circuit diagram for MQ-series sensors. This shows how the sensor uses $R_S$ and $R_L$ to make a voltage divider.}
\label{circuit}
\end{figure}
...
\begin{figure}[!tb]
\centering
\includegraphics[width=0.4\textwidth]{images/potentiometer}
...
\begin{subfigure}[b]{0.45\textwidth}
  \includegraphics[width=\textwidth]{images/ozone_data_sheet}
  \caption{MQ131.}
  \label{mq131_curve}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.45\textwidth}
  \includegraphics[width=\textwidth]{images/mq135_data_sheet}
  \caption{MQ135.}
  \label{mq135_curve}
\end{subfigure}
...
\section{Limitations} \label{calibration_limitations}
...
It is also worth noting that the data sheets for these sensors indicate that their outputs can drift over time. Our data collection period is 2 weeks, which is too short to notice a change as they typically occur over bi-annual or annual periods of time. However, this effect should be tested and accounted for if the device is to be used for longer periods of time.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%							Question 1					      		       %%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\chapter{Monitoring air pollution in Bath} \label{chap:q1}

Chapters \ref{chap: system_design} and \ref{chap: calibration} detail how the air quality monitoring device, and system as a whole, is constructed and how the sensors are calibrated to output meaningful values. Those chapters collectively describe the process undertaken in creating the final device, providing a product that can now be used to answer our research questions. Consequently, this chapter focuses on the first research question:
\newline\newline
\noindent\textit{\textbf{Research Question 1:} Can air pollution be monitored to a satisfactory level of accuracy using a bicycle-mounted monitoring device?}
\newline

Section \ref{meth:q1} details the methodology behind how the device is applied to answer this research question. Each method is described in detail, with our reasoning provided in each case to support our choices. Section \ref{results:q1} then displays the results and analyses the findings.

\section{Methodology} \label{meth:q1}

The methodology is structured as follows. The first three parts focus on where and how the data is collected, which includes the location choice (Section \ref{location}), the selection of the route (Section \ref{route_selection}) and the data collection method (Section \ref{data_collection:q1}). In order to then answer the research question, a number of distinct parts need to be considered. Firstly, the optimal positioning of the device on the bicycle needs to be considered (Section \ref{position_testing}). Secondly, the quality of the data from this device must be assessed to test whether it is satisfactory and any necessary changes made (Section \ref{data_quality}). Thirdly, the usefulness of this data needs to assessed via the creation of pollution maps (Section \ref{pollution_mapping}) and pattern identification (Section \ref{identifying_patterns}). Finally, the data needs to compared with the measurements from the current fixed monitoring stations (Section \ref{identifying_patterns}).
...
\begin{figure}[!tb]
  \centering
  \includegraphics[width=0.7\linewidth]{images/cycling_route}
...
\begin{figure}[!tb]
\centering
\includegraphics[width=1\textwidth]{images/shaded_route}
...
To collect data, we ride a bicycle with the air quality monitoring device attached around a pre-determined route, detailed in Section \ref{route_selection}. The route is traversed in both directions in order to help ameliorate the effect of anomalous measurements. This leads to minor differences in the route due to certain one-way streets in Bath. We ride the bicycle at a relatively constant speed in order to distribute measurements regularly in spatial terms.

Data is collected across 3 weeks in July and August, from Monday 23rd July to Tuesday 14th August. The route is cycled both in the morning and afternoon. The morning run is to coincide with the morning rush hour and the evening run is to coincide with the evening rush hour. These times are deemed to have the highest pollution levels across the day.

\subsection{Positioning on bicycle} \label{position_testing}

In order to identify which position on a bicycle is optimal for an air quality monitoring device, we need to test the device attached to different positions. We identify three potential positions on a bicycle, shown in Figure \ref{bicycle_positions}. Position 1 is situated directly below the rider and could potentially be impacted by particles thrown up by the front wheel. It is also not as visible to the user. On the other hand, this position may provide good protection from the elements, improving robustness. Position 2 is the least visible, but is situated lowest on the bicycle frame and, thus, provides a different testing environment for the sensor. This may also be impacted by particles thrown up by the wheels. Finally, Position 3 is situated above the front wheel, thus avoiding any particles thrown up by the wheels. In addition, it is the most suitable position for the user to view the device, allowing the user to notice when the system-status LED is flashing. 

\begin{figure}[!tb]
\centering
\includegraphics[width=0.6\textwidth]{images/bicycle_positions}
\caption[Device positions on bicycle.]{Potential positions for device on bicycle. (1) is directly below the rider, (2) is on the seat stays, and (3) is above the front wheel.}
...
In order to identify more precisely which measurements are outliers in our data, a three-part approach is used. In each case, we apply the outlier detection method and if all of these methods report a reading to be an outlier, it is filtered out from the sample through the use of a type of band pass filter.
\begin{enumerate}
\item Upper/lower quartiles and Inter-quartile range (IQR)
\item Median and Median Absolute Deviation (MAD)
\item Median and S\textsubscript{n} (an alternative estimator to MAD, suggested by \cite{rousseeuw1993alternatives})
\end{enumerate}

These methods are run on three different groupings to assess which is best: (i) on the dataset as a whole, (ii) by time period grouping, and (iii) by a moving time period. Each of these methods are explained below. 
...
These two values can then be used to identify outliers by calculating the following statistic for each $x_i$ and flagging those $x_i$ for which this statistic exceeds a certain cutoff (typically 2.5 or 3):
\begin{equation} \label{eqn:mad_stat}
...
Equation \ref{eqn:sn} is very similar to Equation \ref{eqn:mad}, except that the median function $M_j(\cdot)$ has been moved outside the absolute value. The breakdown point of estimator $S_n$ is still 50\%, but it has a better efficiency than MAD. The same test statistic as detailed in Equation \ref{eqn:mad_stat} is then used to identify outliers, but with MAD replaced with S\textsubscript{n}:
\begin{equation} \label{eqn:sn_stat}
\frac{| x_i - M_j(x_j) |}{S_n}
\end{equation}
...
\subsection{Pollution mapping} \label{pollution_mapping}

With the pollution data collected along the route detailed in Section \ref{route_selection}, we can create pollution maps that show how pollution varies along the route. To achieve this, a grid of tiles overlay is placed on top of the map, combining mapping ideas from both \cite{Hoang2013hanoihexagons} and \cite{Hasenfratz2015highresmapsTram}. A $50 \times 50$ grid of tiles will be used to cover an area of $2.7~km \times 2.7~km$, which makes each tile $55~m \times 55~m$ in size. Each tile's colour is an aggregation of all values that fall inside its boundaries -- median and maximum aggregations are used for this. We use two colouring schemes: (i) a green-to-red gradient as \cite{smallbone2012customerinsight} found that the public prefer a blue or green colour for representing good air quality and red for representing poor air quality, and (ii) a DAQI colour scheme \citep{defra2018daqi}.

\subsection{Identifying patterns} \label{identifying_patterns}
...
\begin{figure}[!tb]
\centering
\includegraphics[width=0.7\textwidth]{images/roads}
...
\begin{table}[!tbp]
  ...
  \toprule
  {} & \multicolumn{4}{c}{PM\textsubscript{10}  ($\mu g/m^3$)} \\
  \cmidrule(lr){2-5}
...
As Figure \ref{fig:pm_histogram} shows, the distribution of PM\textsubscript{10} measurements is positively skewed and it appears that the measurements contain a large number of outliers. The majority of the values are distributed between 0 and 200, with a number of other values spread out to over \num{1500}. Within the frequent observations, the positive skew indicates that lower measurements tend to be more common than higher measurements. Compared to the other histograms, the PM\textsubscript{10} measurements from the Shinyei PPD42 particulate sensor have the largest range of values and, consequently, potentially the largest proportion of outliers.
...
\begin{table}[!tbp]
  ...
  PM\textsubscript{10}	& 9877	& 9089	& 788	& 8.0		& 8063	& 1026	& 11.3 \\
...
%  \cmidrule(lr){6-8}
  ...
  PM\textsubscript{10} ($\mu g/m^3$)	& 128.64	& 251.18	& 1768.13	 \\
...
In addition, Figure \ref{fig:daqi_maps} shows the same maps using the UK's DAQI colouring scheme. Only PM\textsubscript{10} and O\textsubscript{3} are shown as the MQ135 measures general air quality using voltage and this does not correspond to the DAQI. The median graphs show that the PM\textsubscript{10} and O\textsubscript{3} pollution levels are `Low' on average, despite Figure \ref{fig:pm10_map_median} and Figure \ref{fig:o3_map_median} showing variations in the values. In addition, the maximum graph for PM\textsubscript{10} (Figure \ref{fig:pm10_map_max_daqi}) indicates that the majority of the route has been measured as `High' or `Very High' at least once. On the other hand, Figure \ref{fig:o3_map_max_daqi} shows that the maximum ozone level around the route is always `Low'.

\begin{figure}[!tb]
    \centering
    \begin{minipage}{1\linewidth}
            \begin{subfigure}[t]{.5\linewidth}
                \includegraphics[width=\textwidth]{images/pm10_map_median_daqi}
                \caption{PM\textsubscript{10} (median)}
                \label{fig:pm10_map_median_daqi}
            \end{subfigure}
            \begin{subfigure}[t]{.5\linewidth}
            	\includegraphics[width=\textwidth]{images/pm10_map_max_daqi}
            	\caption{PM\textsubscript{10} (max)}
            	\label{fig:pm10_map_max_daqi}
	   \end{subfigure}
            \begin{subfigure}[t]{.5\linewidth}
                \includegraphics[width=\textwidth]{images/o3_map_median_daqi}
                \caption{O\textsubscript{3} (median)}
                \label{fig:o3_map_median_daqi}
            \end{subfigure}
            \begin{subfigure}[t]{.5\linewidth}
            	\includegraphics[width=\textwidth]{images/o3_map_max_daqi}
            	\caption{O\textsubscript{3} (max)}
...
Figure \ref{fig:dow_bars} shows the distribution of pollution levels for each pollutant by the day of the week. As Figure \ref{fig:dow_pm10} shows, the average PM\textsubscript{10} pollution level increases throughout the week, from a low on Monday of 20.3~$\mu g/m^3$ to a high on Sunday of 32.1~$\mu g/m^3$. Table \ref{tab:dow} shows the results of regressing the pollution measurements on the days of the week, with Monday as the reference category (to avoid the dummy variable trap/multicollinearity) and the remaining days as dummy variables. The results indicate that Tuesday to Thursday have no significant effect for PM\textsubscript{10}, but Friday, Saturday and 
...
\toprule
Variable & \multicolumn{1}{c}{PM\textsubscript{10}} & \multicolumn{1}{c}{Ozone} & \multicolumn{1}{c}{General AQ} \\ 
\midrule
...
\noindent Sunday are all significant at the 1\% significance level, with the coefficient ranging from 6.28 to 16.99~$\mu g/m^3$. Although the coefficients on all of the days are significant for ozone, there is no clear pattern. The only noticeable effect is a reduction of 5.18~$\mu g/m^3$ on a Saturday. For the general air quality measurements, all coefficients are negative and significant, with the largest effect being a reduction of 0.14~V at the weekend.
...
\begin{figure}[H]
    ...
    \minipage{0.33\textwidth}
      \begin{subfigure}[t]{\linewidth}
      \includegraphics[width=\linewidth]{images/am_pm10}
      \caption{PM\textsubscript{10}}
      \label{fig:am_pm10}
      \end{subfigure}
    \endminipage\hfill
    \minipage{0.33\textwidth}
      \begin{subfigure}[t]{\linewidth}
      \includegraphics[width=\linewidth]{images/am_o3}
      \caption{O\textsubscript{3}}
      \label{fig:am_o3}
      \end{subfigure}
    \endminipage\hfill
    \minipage{0.33\textwidth}
      \begin{subfigure}[t]{\linewidth}
      \includegraphics[width=\linewidth]{images/am_mq135}
      \caption{MQ135}
      \label{fig:am_mq135}
      \end{subfigure}
    \endminipage
...
\begin{tabular}{l S[table-format=2.3] !{\qquad} S[table-format=2.3] !{\qquad} S[table-format=1.3]}
\toprule
Variable & \multicolumn{1}{l}{PM\textsubscript{10}} & \multicolumn{1}{l}{Ozone} & \multicolumn{1}{c}{General AQ} \\ 
\midrule
...
Table \ref{tab:temp_hum} details the results of regressing each pollutant on temperature and humidity. These variables are significant in all cases. The effect on PM\textsubscript{10} measurements is negative, with a \SI{1}{\celsius} increase in temperature leading to a 1.24~$\mu g/m^3$ decrease in PM\textsubscript{10} and a 1 percentage point increase in humidity leading to a 0.14~$\mu g/m^3$ decrease in PM\textsubscript{10}. The effect on ozone and general air quality measurements is positive for both temperature and humidity. The magnitude of the effects is similar for ozone with 1.17 and 0.13~$\mu g/m^3$ increases for unit increases in temperature and humidity, respectively. For general air quality, these effects are 0.015~V and 0.004~V.
...
The results from comparing the bicycle-mounted device's measurements with those from the fixed monitoring stations are mixed. The PM\textsubscript{10} results, which are for the Windsor Bridge location only, show a slightly negative relationship between the two sets of measurements. The device's measurements also have a negative relationship with the Guildhall's NO\textsubscript{x} measurements. On the other hand, the Windsor Bridge and London Road locations show positive relationships for NO\textsubscript{x}, with corresponding correlations of 0.54 and 0.27.

\begin{figure}[!tb]
	\begin{subfigure}[t]{.5\linewidth}
		\includegraphics[width=\textwidth]{images/comparison_pm10}
                \caption{PM\textsubscript{10}}
                \label{fig:comparison_pm10}
	\end{subfigure}
	\begin{subfigure}[t]{.5\linewidth}
		\includegraphics[width=\textwidth]{images/comparison_nox}
...
\section{Discussion} \label{disc:q1}
...
As the last column of Table \ref{tab:upper_bounds} shows, using outlier detection methods with moving averages is unwise. This is because if there are a relatively high number of consecutive outliers, the moving average window's statistics are greatly skewed and measurements that would otherwise be considered extreme outliers are not excluded. Using moving averages on the PM\textsubscript{10} measurements is particularly bad due to the abnormally high proportion of outliers.
...
The results from the comparison between fixed monitoring stations and the bicycle-mounted device are mixed. The problem with this comparison is that it is difficult to compare measurements from a moving device with a stationary point. Consequently, the results from this are not discussed further. Future comparisons should be conducted against a different `ground-truth', for example, collecting measurements from along the same route using an industry-grade sensor.
...
\chapter{Improving spatial \& temporal resolution}  \label{chap:q2}

Chapter \ref{chap:q1} showed that this air quality monitoring device can measure air quality to a satisfactory level, meaning it can identify trends and variations in air pollution. Consequently, the second research question focuses on whether this device can improve on the spatial and temporal resolution of the fixed air quality monitoring stations.
\newline\newline
\textit{\textbf{Research Question 2:} Can such a device improve spatial and temporal resolution of air quality measurements compared to fixed monitoring stations?}
\newline

Answering this question will help to ascertain whether such bicycle-mounted devices can replace their more expensive, fixed counterparts. Section \ref{meth:q2} details the methodology behind answering this research question and then Section \ref{results:q2} displays the results and analyses the findings.
...
A fixed monitoring station can only take measurements in exactly one location. Whilst they do typically use high-quality, expensive monitoring equipment that offers higher accuracy and reliability, they have poor spatial resolution. Our bicycle-mounted air quality monitoring device is portable and so can take measurements at varying locations with ease. To demonstrate this improvement in spatial resolution, we need a suitable location and data collection method. 
...
The data provides a pollution level along the route for every 7-minute interval, which can then be used to show how the pollution changes over the whole time period. We implement a heatmap visualisation technique used by \cite{Peters2013cycleruns}, which provides a pollution gradient at each time interval for the route outside the football stadium. The results of this are shown in Section \ref{results:q2}.
%This more granular picture of air quality over time demonstrates the improved temporal resolution that this device can offer, as well as proving its use in monitoring events whilst maintaining portability.
...
We now discuss the results for Research Question 2. Section \ref{spatial} focuses on the improvements in spatial resolution, whilst Section \ref{temporal} discusses the temporal resolution aspect.

\subsection{Spatial resolution} \label{spatial}

Covering a larger area and improving spatial resolution are only beneficial if the air quality measurements vary across this spatial area. For example, if all of the measurements collected around the Guildhall fixed monitoring station had little variation, then this increase in spatial resolution actually adds no benefit. This is because the fixed monitoring station's measurements apply approximately equally to the surrounding area. Therefore, the maps generated by the locus-based monitoring need to demonstrate that the device can register varying pollution levels over short distances. Previous research has shown that pollution can indeed vary over extremely short distances \citep{hewitt1991spatialvar,wilson2006intraurbanvar}. In particular, `street canyons' can trap pollution on the street whilst behind these buildings the level drops to the background pollution level. We find that pollution does vary over short distances (Figure \ref{fig:locus_maps}), both when using the median and maximum to aggregate measurements within each tile. This implies that the improved spatial resolution found in this project is beneficial as it allows one to better understand how pollution varies across the urban environment. However, there is not a significant difference [TEST STAT] between the median and maximum pollution levels [???]. 

\cite{Elen2013aeroflex} also demonstrated an improved spatial resolution when riding a bicycle-mounted air quality monitoring device around a section of Antwerp, Belgium. The maps generated by this device demonstrated variations of up to a factor of 10 between measurements on the main road (where the fixed monitoring site was located) and measurements from nearby minor roads. Whilst our device does not register such great variations, their device was built using much more expensive sensors, which likely has greater accuracy. Nonetheless, this supports our finding that the bicycle-mounted monitoring can improve spatial resolution and that the improved resolution is beneficial.

In addition, it is also interesting to note that the pollution level is lower on the bridge over the river. One potential cause for this could be the greater exposure to the wind. However, without sufficient wind speed and direction data, a causal relationship cannot be established.

\subsection{Temporal resolution} \label{temporal}
...
Bath's roads have a total length of 52.8~km, as calculated by the \texttt{osmnx} python module \citep{boeing2017osmnx} using OpenStreetMap data (see Section \ref{network_graph} for further discussion of this module). Given the resource constraints of this project, it is unfeasible to collect pollution data across the whole city whilst only using one cyclist. Moreover, collecting any more than one time period of data would be even more difficult. Consequently, we simulate the pollution data across Bath. We section the city into a raster of 50 by 50 cells, assigning a pollution value to each cell.
...
In this chapter, we conclude the project in Section \ref{conclusion}, as well as detailing the limitations of this project (Section \ref{limitations}). We then provide recommendations for future work in Section \ref{future_work}, as well as an additional note on a Mongolian project in which the device is being used (Section \ref{mongolian_project}).

\section{Conclusions} \label{conclusion}
% Define my contribution to the field
% In this thesis, X major contributions have been made...

Air pollution is a major problem worldwide and leads to the deaths of millions of people each year \citep{WHO2016ambient}. However, despite the decrease in the cost of computers and the increasing prevalence of low-cost sensors, monitoring solutions remain relatively expensive and are frequently placed in fixed locations, which offer poor spatial resolution. There are currently very few mobile air quality monitoring devices in the literature, particularly using bicycles. Of those that are attached to bicycles, none is low-cost.

We address this problem by designing and constructing a low-cost, bicycle-mounted air quality monitoring device using low-cost pollution sensors and a Raspberry Pi, along with a GPS module that enables geo-tagging of pollution measurements. Firstly, we show that a bicycle-mounted monitoring device can measure air pollution to a satisfactory level. The device can accurately detect variations in pollution level and the data can be used to identify patterns. For example, we found that the PM\textsubscript{10} level increases at the end of the week and pollution is higher in the evenings. Overall, the average levels of pollution in Bath were found to be `Low' when using the UK's DAQI. In addition, we find that the optimal position for the device is under the headtube at the front of the bicycle. We then demonstrate that using such a device can improve both spatial and temporal resolution in comparison to the current fixed monitoring stations in Bath. We create pollution maps with high spatial resolution of $55~m \times 55~m$ around the Bath Guildhall fixed monitoring station, as well as a high temporal resolution of 7 minutes at a Bath City FC football event. Additionally, we discover a novel application of the device to event-based air quality monitoring, which could be particularly beneficial to local authorities in the UK. Finally, we design a novel cost function that incorporates the effect of road gradient on inhalation of air pollution and combine this with Dijkstra's algorithm to generate alternative health-optimal routes. Choosing the health-optimal route instead of the shortest route leads to an average reduction in pollution exposure of 5.08\%, at the cost of a 4.39\% increase in route length. Using a recent study on the health impact of a reduction in PM\textsubscript{10} exposure \citep{Ebenstein2017lifeexpectancy}, our findings equate to an increase in life expectancy of 0.08 years if the health-optimal route is chosen consistently.

The global issue of air pollution is only going to increase in prevalence in the future and so having a ubiquitous monitoring system is imperative. We believe that the device created in this project provides a good first prototype for such a device. The benefits of a mobile monitoring device are numerous and a sufficient collection of such devices will enable detailed, widespread air quality monitoring on a scale that has previously been impossible due to cost and operational constraints.

However, the usefulness of the device partly depends on the degree to which public preferences can be changed. Only a fraction of the public cycle and a major concern of commuters is journey time. Therefore, if preferences towards cycling and attitudes on air pollution are not changed, it is unlikely that sufficient coverage can be achieved. One solution to this problem is to build more cycle paths to increase the use of bicycles and to provide education on the effects of pollution. In addition, applications using air quality data should make use of outcome framing to ensure that the public change their behaviour \citep{mir2016framing}.

\section{Limitations} \label{limitations}

Whilst the project has answered the posed research questions, there were some limitations to the project. The limitations surrounding the MQ135 sensor are discussed in Section \ref{calibration_limitations} so are not repeated again here.

Firstly, the data collection was carried out during the summer months of July and August. It has previously been shown that pollution concentrations tend to be lower in the summer than the winter in Europe \citep{Bigi2010summer}. As a result, the pollution level during the project may have been lower than average. Given the time restrictions of the project (June 2018 -- September 2018), it was not possible to collect data over a sufficient time period to experience seasonal differences.

Moreover, the time restriction of the project meant that some additional elements could not be tested. Firstly, the testing of more advanced calibration methods, which were detailed in Section \ref{calibration_techniques}, could have improved the accuracy of the sensor measurements. For instance, the initial output conversion and calibration from this project would remain, but could be augmented by the random forests calibration method. Secondly, the maps generated in this project could be enhanced by using more advanced mapping methods detailed in Section \ref{estimationtechniques}. For example, the use of land-use regression can help to map pollution in un-visited areas, thus generating more complete pollution maps.

%On the other hand, it was also during the school holidays and a UK heatwave, which could have led to many people going on holiday in the UK. Bath is a popular tourist destination, so the traffic level may have been increased, leading to higher pollution levels during this study.

In addition, another main limitation faced during this project was the availability of only one rider and only one constructed air quality monitoring device. Consequently, a limited amount of data could be collected. This was particularly influential in Chapter \ref{chap:q3}, in which we simulated the data in order to investigate the health-optimal routing. This could have impacted the path costs, which in turn would have affected the routes and, therefore, the health effect found.

%Additionally, there was only a limited amount of time available for the data collection. Ideally, the data collection period would have been much longer and involved multiple riders. As can be the case with low-cost sensors, if there is enough data and enough devices, one can obtain high accuracy data.

Finally, the project was hampered by equipment breakages during the data collection. Although the sensors were replaced, low-cost sensors can have slight differences in output, which could have affected the measurements post-replacement.

\section{Future work} \label{future_work}
% Not a shopping list of ideas and potential projects

%As a result of building this air quality monitoring device and conducting thorough testing, we ha identified a number of limitations, detailed in Section \ref{limitations}. These have helped us to narrow our focus and design improvements for future iterations of the device.

Firstly, we propose a number of design improvements for future iterations of the device. We propose re-designing the case of the device to enclose all of the sensors in a miniature controlled environment. The case would have one, or potentially multiple, inlet fans that draw air into the container at a controlled -- and measured -- flow rate. This is for two reasons: (i) the encased design would waterproof the device, making it much less prone to the rain-related issues we encountered in this project, and (ii) to help negate the effects of the speed of the bike, which were shown in Section \ref{patterns} to influence the sensor readings. Secondly, we would like to replace the Shinyei PPD42 sensor with a more reliable alternative. This is because of the high percentage of outliers in its measurements.

One of the initial project aims was to relate the pollution data with traffic data. However, it became clear during the project that high resolution traffic data was not available in Bath, only annual average traffic flow data. Therefore, if more granular traffic data could be obtained in the future, it would be interesting to test whether there is a relationship between traffic and the air quality data collected by this project's device.

Finally, the aforementioned advanced calibration methods and mapping techniques should be tested. The advanced calibration methods would enable high accuracy measurements from low-cost sensors, whilst the more advanced mapping techniques would enable more complete pollution mapping in un-visited areas.

%It would also be beneficial to test the use of more advanced calibration methods that were detailed in Section \ref{calibration_techniques}, but that there was not enough time to implement. The initial output conversion and calibration from this project would remain, but the more advanced techniques could be used to re-calibrate the sensors in real time...
%
%Finally, the maps generated in this project could be enhanced by using more advanced mapping methods detailed in Section \ref{estimationtechniques}. For example, the use of land-use regression can help to map pollution in un-visited areas, thus generating more complete maps.
...
\section{Additional application post-project completion} \label{mongolian_project}
...
Four main adjustments were made to the device to enable this application. Firstly, the triangular bicycle bag was replaced with a plastic container and a fan fitted into the wall of the container to induce air flow over the sensors. This makes the device relatively waterproof as the sensors are no longer directly exposed to the environment. Secondly, we replaced the MQ135 sensor with the dedicated NO\textsubscript{2} sensor that was recently released by MikroElektronika (NO\textsubscript{2} click). There was not sufficient time before the team flew to Mongolia to calibrate the NO\textsubscript{2} sensor, so the readings are stored as raw voltages. This should enable accurate NO\textsubscript{2} measurements, which can then be converted from voltages to NO\textsubscript{2} readings in the future once the sensor has been calibrated. Thirdly, we secured all sensors physically inside the container, thereby avoiding power disruption issues from vibrations and making the device more robust. Finally, functionality for HTTP posting the CSV files to a database was removed and replaced by saving all readings to one continuous CSV file, since a reliable internet connection would not be available in Mongolia. Figure \ref{fig:modified_device} shows the modified device.

Unfortunately, the results will not be available until after project completion, but this further testing will be beneficial for two reasons: (i) it tests the new container and NO\textsubscript{2} sensor, and (ii) the device is being tested in extremely high levels of pollution.
...
\section{Wi-Fi configuration file} \label{wpa_supplicant}

\lstinputlisting{wpa_supplicant.conf}


\chapter{Outliers} \label{appendix:outliers}

The results of performing this outlier detection method are shown in Table \ref{tab:iqr_outliers}. It is clear from the table that the PM\textsubscript{10} measurements have the highest percentage of outliers (11.75\%) compared to the ozone (1.79\%) and general air quality measurements (1.97\%). The upper bound of 128.64 $\mu g/m^3$ is reasonable, given that the highest PM\textsubscript{10} measurement recorded in 2017 by the Windsor Bridge monitoring station in Bath was 204 $\mu g/m^3$ \citep{courthold2018max}. The MQ131 sensor has the lowest percentage of outliers when using the IQR detection method. It must be noted that the sample size $N$ for the ozone measurements is considerably lower than for the other two pollutants due to the aforementioned breakage and replacement of the MQ131 sensor midway through data collection. However, the percentage of outliers is similar to that for the general air quality measurements, so this exclusion does not appear to have detrimentally impacted the analysis from a statistical perspective.

The second outlier detection method is the median and MAD method. Outliers were identified as any measurements which were more than 3 times the MAD away from the median. The results of this method are detailed in Table \ref{tab:mad_outliers}. Once again, PM\textsubscript{10} shows the highest percentage of outliers, with 14.14\% of the measurements being considered outliers. This is a 2.39\% increase over the number of outliers found by the IQR detection method. On the other hand, there has been a small decrease in the outlier percentages for the ozone and general air quality measurements. However, overall, the values are not too dissimilar to those found by the IQR method, which indicates the robustness of these methods.

The third method uses the median and S\textsubscript{n} and the results are shown in Table \ref{tab:mad_outliers}. The PM\textsubscript{10} outlier percentage is only 0.07\% less than using the MAD method. The percentage of outliers for ozone and general air quality are the lowest out of the three methods.
...
\begin{table}[!tbp]
  ...
  \toprule
  Gas/particulate & N & {Median} & {IQR} & {U.B.} & {L.B.*} & {\# outliers} & {\%} \\ \midrule
  PM\textsubscript{10} $(\mu g/m^3)$ & 9089 & 28.13 & 45.50 & 128.64 & 0.00 & 1068 & 11.75 \\
  O\textsubscript{3} $(\mu g/m^3)$ & 7920 & 50.31 & 11.17 & 72.74 & 28.06 & 142 & 1.79 \\
  General AQ (V) & 9877 & 1.17 & 0.21 & 1.60 & 0.74 & 195 & 1.97 \\ \bottomrule
    \multicolumn{7}{l}{\textsuperscript{$\ast$}\footnotesize{Lower bound (L.B.) capped at 0 as negative concentrations are nonsensical.}} \\
  \end{tabular}
...
\begin{table}[!tbp]
  ...
  \toprule
  Gas/particulate & N & {Median} & {MAD} & {U.B.} & {L.B.*} & {\# outliers} & {\%} \\ \midrule
  PM\textsubscript{10} $(\mu g/m^3)$ & 9089 & 28.13 & 25.38 & 104.26 & 0.00 & 1285 & 14.14 \\
  O\textsubscript{3} $(\mu g/m^3)$ & 7920 & 50.31 & 8.30 & 75.21 & 25.42 & 105 & 1.33 \\
  General AQ (V) & 9877 & 1.17 & 0.16 & 1.65 & 0.69 & 139 & 1.41 \\ \bottomrule
    \multicolumn{7}{l}{\textsuperscript{$\ast$}\footnotesize{Lower bound (L.B.) capped at 0 as negative concentrations are nonsensical.}} \\
  \end{tabular}
...
\begin{table}[!tbp]
  ...
  \toprule
  Gas/particulate & N & {Median} & {S\textsubscript{n}} & {U.B.} & {L.B.*} & {\# outliers} & {\%} \\ \midrule
  PM\textsubscript{10} $(\mu g/m^3)$ & 9089 & 28.13 & 25.22 & 103.80 & 0.00 & 1288 & 14.17 \\
  O\textsubscript{3} $(\mu g/m^3)$ & 7920 & 50.31 & 8.46 & 75.69 & 24.94 & 97 & 1.22 \\
  General AQ (V) & 9877 & 1.17 & 0.17 & 1.67 & 0.67 & 121 & 1.23 \\ \bottomrule
    \multicolumn{7}{l}{\textsuperscript{$\ast$}\footnotesize{Lower bound (L.B.) capped at 0 as negative concentrations are nonsensical.}} \\
  \end{tabular}
...
\section{PHP API} \label{code:php_api}

\lstinputlisting{route_api.php}

\section{Web application using R Shiny} \label{code:web_app}
